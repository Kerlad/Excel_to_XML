...
class MainWindow(QMainWindow):
    ...
    def show_help(self):
        ...
        Минтруда. Каждая запись содержит: ФИО, СНИЛС, номер и название программы,
        дату экзамена, номер протокола, SetId и регистрационный номер (baseNo).</p>
        ...
        <p><b>Данные протокола:</b> введите номер протокола. Кнопка «Программы обучения»
        открывает окно для редактирования номеров документов и часов обучения
        по каждой программе (двойной клик по ячейке).</p>
        <p><b>Данные комиссии:</b> заполните название организации, номер и дату приказа,
        ФИО и должности председателя, членов комиссии №1, №2, №3 и представителя
        профсоюза. Нажмите «Сохранить данные комиссии» — данные сохранятся между
        сессиями. Кнопка «Загрузить данные комиссии» восстановит ранее сохранённые
        данные.</p>
        <p><b>Генерация протокола:</b> нажмите «Сгенерировать протокол» — система
        заполнит шаблон <code>Protokol_proverki_znanii_OT.xlsx</code> данными
        комиссии и сохранит итоговый файл. Обязательные поля: номер протокола,
