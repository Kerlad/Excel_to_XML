...
class BaseDataTransferTab(QWidget):
    """
    Базовый класс вкладки передачи данных.
    Подклассы должны реализовать метод _get_api_module().
    """
    def __init__(self, tab_title="Передача данных", api_module=None):
        super().__init__()
        ...
        # Callback-функции для журнала
        self._journal_add_callback = None
        self._journal_update_callback = None

    def _get_api_module(self):
        """Возвращает модуль API. Должен быть переопределен в подклассах."""
        raise NotImplementedError("Подкласс должен реализовать _get_api_module()")
    ...
    def save_api_key(self):
        ...
        from api.mintrud_api import save_api_key
    ...
    def send_xml(self):
        ...
        if api_module is None:
            QMessageBox.critical(self, "Ошибка", "API модуль не загружен")
            return
        ...
        result = api_module.push_xml(api_key, xml_file, proxy_settings=proxy_settings)
    ...
    def query_set_id(self):
        ...
        proxy_settings = self._get_proxy_settings()
        result = api_module.get_by_set_id(api_key, set_id, proxy_settings=proxy_settings)
        ...
        if file_path:
            ok, msg = api_module.export_records_to_xlsx(records, file_path)
            if ok:
    ...
    def query_snils(self):
        ...
        proxy_settings = self._get_proxy_settings()
        result = api_module.get_by_snils(api_key, snils, proxy_settings=proxy_settings)
