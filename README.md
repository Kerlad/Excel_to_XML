...
**Сайт программы:** [normaot.ru](https://normaot.ru/)
...
- Поля ИНН/наименование УЦ и Заказчика в XLSX можно оставлять пустыми — затем заполнить одной кнопкой «Заполнить УЦ и Заказчика» на вкладке «Просмотр данных»
- Без заполненных УЦ и Заказчика конвертация в XML невозможна (выдаётся предупреждение)
...
- **Корпоративные прокси**: автоопределение среды РЖД (`.rzd`, `.oao`), приоритет WinINET, автоматический обработчик SSL-инспекции с подтверждением пользователя
...
- Диалог прогресса при запросе из реестра: индикация прогресса, секундомер, кнопка отмены
...
- Неблокирующие всплывающие уведомления (toast) для подтверждений вместо модальных окон — не прерывают работу
- Единая дизайн-система кнопок (primary / success / danger) для светлой и тёмной темы
...
| `Ctrl+T` | Создать шаблон XLSX |
| `Ctrl+E` | Экспорт всех данных XLSX |
| `Ctrl+L` | Заблокировать сессию |
...
| 3 | Проверьте данные и нажмите «Заполнить УЦ и Заказчика» если нужно подставить настройки | Просмотр данных |
| 4 | Нажмите «Конвертировать» (если поля УЦ/Заказчика пусты — будет предупреждение) | Просмотр данных |
| 5 | Выберите XML и отправьте на сервер (нужен API-ключ) | Передача данных |
| 6 | Получите SetId → запросите рег. номера | Передача данных |
| 7 | Сформируйте протокол проверки знаний (DOCX) | Журнал проверки знаний |
| 8 | Проверяйте статусы сотрудников через сводку | Справка о работниках |
