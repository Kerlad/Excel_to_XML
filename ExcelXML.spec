# -*- mode: python ; coding: utf-8 -*-
import sys
import os

block_cipher = None

a = Analysis(
    ['main.py'],
    pathex=[],
    binaries=[],
    datas=[
        ('ico.ico', '.'),
        ('schema', 'schema'),
        ('data', 'data'),
        ('Protokol_proverki_znanii_OT.docx', '.'),
    ],
    hiddenimports=[
        # PySide6
        'PySide6',
        'PySide6.QtCore',
        'PySide6.QtGui',
        'PySide6.QtWidgets',
        # Сторонние библиотеки
        'openpyxl',
        'openpyxl.cell',
        'openpyxl.workbook',
        'openpyxl.styles',
        'openpyxl.utils',
        'xlrd',
        'xlrd.xldate',
        'lxml',
        'lxml.etree',
        'requests',
        'requests_ntlm',
        'requests_ntlm.HttpNtlmAuth',
        'requests_negotiate_sspi',
        'requests_negotiate_sspi.HttpNegotiateAuth',
        'win32security',
        'win32api',
        'pywintypes',
        'sspicon',
        'urllib3',
        'urllib3.exceptions',
        'cryptography',
        'cryptography.fernet',
        'PIL',
        'PIL.Image',
        'docx',
        'docx.shared',
        'docx.oxml',
        'docx.oxml.ns',
        # Модули проекта
        'utils',
        'utils.logger',
        'utils.crypto',
        'utils.proxy_manager',
        'utils.tahoe_style',
        'tabs',
        'tabs.data_entry_tab',
        'tabs.data_view_tab',
        'tabs.data_transfer_tab',
        'tabs.exam_journal_tab',
        'tabs.protocol_tab',
        'tabs.programs_dialog',
        'api',
        'api.mintrud_api',
        'journal',
        'journal.journal_manager',
        'protocol',
        'protocol.commission_manager',
        'protocol.programs_manager',
        'exporters',
        'exporters.xml_exporter',
        'exporters.protocol_exporter',
        'importers',
        'importers.xlsx_importer',
        'importers.xml_importer',
        'api.backends',
        'api.backends.base_backend',
        'api.backends.requests_backend',
        'api.backends.urllib_backend',
        'api.backends.httpx_backend',
        'api.backends.pycurl_backend',
        'api.backends.wininet_backend',
        'api.payload_builder',
        'api.response_parser',
        'tabs.single_worker_protocol_tab',
        'tabs.data_transfer_tab_base',
        'tabs.employee_summary_tab',
        'importers.error_report',
        'network',
        'network.client',
        'db',
        'db.database',
        'db.schema',
        'db.workers_data_repo',
        'db.exam_journal_repo',
        'db.employees_repo',
        'db.employee_programs_repo',
        'dateutil',
        'dateutil.relativedelta',
    ],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[
        'numpy', 'pandas', 'matplotlib', 'scipy', 'sympy',
        'notebook', 'jupyter', 'ipython', 'ipykernel',
        'tkinter', 'test', 'unittest', 'pydoc',
    ],
    win_no_prefer_redirects=False,
    win_private_assemblies=False,
    cipher=block_cipher,
    noarchive=False,
)

pyz = PYZ(a.pure, a.zipped_data, cipher=block_cipher)

exe = EXE(
    pyz,
    a.scripts,
    [],
    exclude_binaries=True,
    name='ExcelXML-Mintrud',
    debug=False,
    bootloader_ignore_signals=False,
    strip=False,
    upx=False,
    console=False,
    disable_windowed_traceback=False,
    argv_emulation=False,
    target_arch=None,
    codesign_identity=None,
    entitlements_file=None,
    icon='ico.ico',
    version='version_info.txt',
)

coll = COLLECT(
    exe,
    a.binaries,
    a.zipfiles,
    a.datas,
    strip=False,
    upx=False,
    upx_exclude=[],
    name='ExcelXML-Mintrud',
)
